...
class Controller(udi_interface.Node):
    ...
    def parameterHandler(self, params):
        ...
        if self.Parameters['nodes'] is not None:
            ...
            LOGGER.error(
                'Missing number of node parameter node 1-6 and ip_0-5 your ip address')
    ...
    def createChildren(self, how_many):
        ...
        LOGGER.info('Creating {} Nodes'.format(how_many))
        for i in range(0, how_many):
            address = 'zone_{}'.format(i)
            title = 'BASpi6U6R {}'.format(i)
            ip = self.Parameters.ip_1
            LOGGER.info(ip)
            ip1 = self.Parameters.ip_2
            LOGGER.info(ip1)
            ip2 = self.Parameters.ip_3
            LOGGER.info(ip2)
            ip3 = self.Parameters.ip_4
            LOGGER.info(ip3)
            ip4 = self.Parameters.ip_5
            LOGGER.info(ip4)
            ip5 = self.Parameters.ip_6
            LOGGER.info(ip5)
            node = bas6u_zone.bas6uNode(
